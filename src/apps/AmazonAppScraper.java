...
//            chromeDriver.navigate().to("https://www.amazon.com");
        ...
        File file = new File(dir+"/UndergroundApps" + dateString + ".csv");
...
//        WebDriver chromeDriver = new FirefoxDriver();
        ...
        // Keep track of number of apps parsed
        Integer appCountDown = 0;
...
//                    String appDollarPrice = matcher.group(1);
                       appDollarPrice = appPriceInfo.replace(",",".");
...
//                  System.out.println("\n" + appInfo.replace("\n"," ") + "\n");
                    ...
                    while (matcher.find()) {
                        if (matcher.group(1).contains("$")) {
                            String appPriceAmt = matcher.group(1);
//                            System.out.print(appPriceAmt.trim() + ", ");
                            app.setAppPrice(appPriceAmt.trim());
                        } else {
...
//                        System.out.print(matcher.group(1).replace(",", "") + ", ");
                    ...
                    String fileTextPrint =  app.printToFile();
                    System.out.println(appNumber + " , " + pageNumber + " , " +  fileTextPrint);
                    bufferedWriter.write(appNumber + " , " + pageNumber + " , " + fileTextPrint );
...
//                    System.out.println();
                    ...
                    if(j+1 > appList.size()){// | (appNumber/appList.size())==pageNumber | appCountDown<1){
//                if(appCountDown<1){
//                if(appNumber>4){
                        j = 0;
...
//        }while (appList.size()>1);
        ...
        try{
            String appFileText = "Name, Creator, Ratings, Stars, 5 stars, 4 stars, 3 stars, 2 stars, 1 star, Release date, Content Rating, In app purchases, Best seller rank, App store rank, app category rank\n";
